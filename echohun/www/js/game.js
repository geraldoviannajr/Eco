class Game {
  ...
  addEvents() {
    ...
    if (device.platform == "browser" && !config.FORCE_TOUCH) {
      ...
      this.canvas.addEventListener("mouseup", (e) => {
        ...
        else if (this.checkControls(this.mousePos.x, this.mousePos.y, true))
          return;
      ...
      this.canvas.addEventListener("touchstart", (e) => {
        ...
        if (this.checkControls(touch.clientX, touch.clientY, false)) return;
      ...
      this.canvas.addEventListener("touchend", (e) => {
        ...
        for (let i = 0; i < e.changedTouches.length; i++) {
          ...
          if (
            this.idTouchPlayerMove >= 0 &&
            touch.identifier === this.idTouchPlayerMove
          ) {
            if (config.DEBUG_INFO)
        ...
        if (this.isMousePressed && this.idTouchPlayerMove >= 0) {
          ...
          for (let i = 0; i < e.touches.length; i++) {
            ...
            if (touch.identifier === this.idTouchPlayerMove) {
              ...
              const scaleY = this.canvas.height / rect.height;
              const mouseX =
                (touch.clientX - rect.left) * scaleX + this.camera.x;
              const mouseY =
                (touch.clientY - rect.top) * scaleY + this.camera.y;
              this.mouseTarget = { x: mouseX, y: mouseY };
              break;
            }
          }
  ...
  static init() {
    ...
    document
      ...
      .addEventListener("touchstart", btnClick);
  }
